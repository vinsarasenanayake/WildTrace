        ...
        <div class="container mx-auto px-6">
            <div class="flex flex-col items-center mb-16 text-center animate-reveal">
                ...
                <div
                    ...
                    <span>BY
                        {{ $product->photographer ? ucwords(strtolower($product->photographer->name)) : 'Unknown' }}</span>
                    ...
                    <div class="bg-white/80 backdrop-blur-xl border border-stone-200 rounded-[2.5rem] p-10 shadow-xl">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="pt-4 border-t border-stone-100/50 mt-6">
                                <label
                                    class="block text-[10px] font-black uppercase tracking-widest text-stone-500 mb-4">Quantity</label>
                                <div class="flex items-center gap-6">
                                    <div
                                        class="flex items-center bg-stone-50/50 border border-stone-200 rounded-2xl p-1.5 shadow-sm">
                                        ...
                                        <span
                                            class="px-8 text-center text-base font-black text-stone-900 tabular-nums">{{ $quantity }}</span>
                                    ...
                                    <div class="flex flex-col">
                                        <span
                                            class="text-[10px] font-bold text-stone-400 uppercase tracking-widest">Total
                                            units</span>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-16 mb-24 items-center">
                <div class="lg:col-span-2 animate-reveal" style="animation-delay: 0.3s;">
                    <h2 class="text-3xl font-serif italic mb-6 text-stone-900 border-l-4 border-green-600 pl-6">Behind
                        the Lens</h2>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-8 py-8 border-y border-stone-200">
                        ...
                        <div>
                            ...
                            <span class="text-lg font-bold italic font-serif text-stone-800">{{ $product->iso }}</span>
                        </div>
                        <div>
                            <span
                ...
                <div class="lg:col-span-1">
                    <div
                        class="relative overflow-hidden rounded-[2.5rem] bg-stone-950 border border-white/5 shadow-2xl shadow-green-900/10 max-w-[380px] mx-auto group">
                        <div class="relative h-[480px] overflow-hidden">
                            <img src="{{ asset($product->photographer->image) }}"
                                class="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105">
                            <div
                                class="absolute inset-0 bg-gradient-to-t from-stone-950 via-stone-950/60 to-transparent">
                            </div>
                            <div class="absolute top-8 right-8 z-20">
                                <span
                                    class="bg-green-600/90 backdrop-blur-md text-white text-[9px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg">{{ $product->photographer->achievement }}</span>
                            </div>
                        </div>
                        <div class="absolute inset-0 flex flex-col justify-end p-8 z-10">
                            <h3 class="text-white text-3xl font-serif italic mb-1">
                                {{ ucwords(strtolower($product->photographer->name)) }}
                            </h3>
                            <p class="text-green-500 text-[10px] font-black uppercase tracking-[0.2em] mb-4">
                                {{ $product->photographer->profession }}</p>

                            <div class="mb-4">
                                <p
                                    class="text-stone-200 text-sm italic leading-relaxed border-l-2 border-green-500/50 pl-4 py-1">
                                    "{{ $product->photographer->quote }}"
                                </p>
                            </div>

                            <div
                                class="mt-2 flex items-center gap-2 uppercase tracking-[0.3em] text-[10px] font-black text-stone-300">
                    ...
                    @foreach($relatedArtifacts as $related)
                        <a href="{{ route('product.show', $related->id) }}"
                            ...
                            <div class="relative overflow-hidden rounded-[2rem] mb-6 shadow-lg aspect-[4/3] w-full">
                                ...
                                <div
                                    class="absolute inset-0 bg-stone-900/10 group-hover:bg-transparent transition-colors duration-500">
                                </div>
                            </div>
                            <div class="flex justify-between items-start px-2 mt-auto">
                                <div>
                                    <h3
                                        class="text-xl font-serif italic text-stone-900 mb-1 group-hover:text-green-600 transition-colors">
                                        {{ $related->title }}</h3>
                                    <p class="text-stone-400 text-[10px] font-black uppercase tracking-widest">
                                        {{ $related->category }}</p>
                                </div>
                                <span
                                    class="text-stone-900 font-black text-sm tracking-widest">${{ $related->price }}</span>
                            </div>
                        </a>
                    @endforeach
                </div>
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4" style="z-index: 100000;">
            ...
            <div class="relative bg-white/95 backdrop-blur-3xl rounded-[2.5rem] shadow-[0_32px_120px_-20px_rgba(0,0,0,0.3)] w-full max-w-xl p-10 overflow-hidden animate-fade-in-up md:p-12 border border-white/20"
                ...
                <div class="text-center mb-10">
                    <img src="{{ asset('images/logo.png') }}" class="h-20 mx-auto mb-6 object-contain hover:scale-110 transition-transform duration-500">
                    <h3 class="text-5xl font-serif italic text-stone-900 mb-2 leading-tight">Welcome Back</h3>
                    <p class="text-[10px] font-black text-stone-400 uppercase tracking-[0.3em] mt-1">Sign in to WildTrace</p>
                </div>
                ...
                <form wire:submit.prevent="performLogin" class="space-y-8">
                    ...
                    <div class="space-y-6">
                        <div class="space-y-2">
                            <label class="text-[11px] font-black uppercase text-stone-500 tracking-[0.1em] ml-1">Email Address</label>
                            ...
                            @error('loginEmail') <span class="text-red-500 text-[10px] mt-2 block font-bold uppercase tracking-wide">{{ $message }}</span> @enderror
                        </div>

                        <div class="space-y-2">
                            <label class="text-[11px] font-black uppercase text-stone-500 tracking-[0.1em] ml-1">Password</label>
                            <div class="relative">
                            ...
                            @error('loginPassword') <span class="text-red-500 text-[10px] mt-2 block font-bold uppercase tracking-wide">{{ $message }}</span> @enderror
                        </div>
                    </div>
